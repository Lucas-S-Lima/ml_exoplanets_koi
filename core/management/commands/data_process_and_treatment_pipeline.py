...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not os.path.exists(input_file):
            logger.error(
                f"Arquivo bruto não encontrado: {input_file}. Execute a task de download primeiro."
            )
            return
        ...
        try:
            ...
            logger.info(
                f"Registros CANDIDATE com zero implausível removidos: {mask_inconsistente.sum()}"
            )
            ...
            if "kepler_object_of_interest_name" in base_data.columns:
                duplicatas_kooi = base_data.duplicated(
                    subset=["kepler_object_of_interest_name"]
                ).sum()
